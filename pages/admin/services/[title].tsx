...
const ServiceAdminPage: FC<Props> = ({
	service,
}) => {
	...
	const deleteUnsavedChanges = async () => {
		try {
			...
			const url =
				'https://todorecsrl-test-dev.s3.sa-east-1.amazonaws.com/';
			const imagesInDB = service.images.map(
				(oneImage) => {
...
export const getServerSideProps: GetServerSideProps =
	async ({ query }) => {
		...
		if (title === 'new') {
			...
			service =
				await dbServices.getServiceByTitle(
					title.toString()
				);
		}
