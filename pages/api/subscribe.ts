...
const subscribeToDb = async (email: string) => {
	try {
		await db.connect();
		const userSubscribeInDB =
			await Subscribe.findOne({ email: email });

		if (
			userSubscribeInDB?.status === 'subscribed'
		) {
			await db.disconnect();
			return {
				message:
...
export const updateSubscribeToDb = async () => {
	...
	try {
		...
		const usersMongo = listUserFromDB.map(
			(eachMember: any) => {
				return {
		...
		const usersMailchimp =
			listUserInMailchimp.members.map(
				(eachMember: any) => {
					return {
		...
		const assignedObjects = Object.assign(
			usersMongo,
			usersMailchimp
		);
		assignedObjects.map(
			async (eachObject: any) => {
